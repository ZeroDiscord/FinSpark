  ...
  try {
    const response = await axios.post(
      ...
      }).toString(),
      {
        ...
        auth: {
          username: config.asana.clientId,
          password: config.asana.clientSecret,
        },
      }
    );
    ...
    console.error('[Asana exchangeCode] status=%d body=%j', err.response?.status, detail);
    throw new AppError(
      `Asana token exchange failed (${err.response?.status}): ${JSON.stringify(detail)}`,
      err.response?.status || 500,
      'ASANA_TOKEN_EXCHANGE_FAILED'
    );
  }
}
...
/**
 * Compute due date from today based on priority.
 * critical → 2 days, high → 5 days, medium → 14 days, low → 30 days
 */
function computeDueDate(priority) {
  const daysMap = { critical: 2, high: 5, medium: 14, low: 30 };
  const days = daysMap[String(priority).toLowerCase()] ?? 14;
  const date = new Date();
...
function buildTaskPayload({ recommendation, projectId, sectionId, dueDate, workspaceId }) {
  ...
  const resolvedDueDate = dueDate || computeDueDate(priority);

  const lines = [
    `${emoji} Priority: ${priorityLabel}`,
    `📋 Status: To Do`,
    `📅 Due: ${resolvedDueDate}`,
    '',
    '─────────────────────────────',
    '🔍 Problem',
    recommendation.problem || 'User friction detected at this feature.',
    '',
    '💡 Suggested Action',
    recommendation.suggestion || 'Review the journey and simplify decision points.',
    '',
    '📊 Metrics',
    recommendation.metrics?.churn_rate !== undefined
      ? `  • Churn Rate: ${Math.round(Number(recommendation.metrics.churn_rate) * 100)}%`
      : null,
    recommendation.metrics?.failure_rate !== undefined
      ? `  • Failure Rate: ${Math.round(Number(recommendation.metrics.failure_rate) * 100)}%`
      : null,
    recommendation.metrics?.drop_off_rate !== undefined
      ? `  • Drop-off Rate: ${Math.round(Number(recommendation.metrics.drop_off_rate) * 100)}%`
      : null,
    '',
    '─────────────────────────────',
  ...
  const data = {
    name: `${emoji} Improve ${recommendation.feature} [${priorityLabel}]`,
    notes: lines.join('\n'),
    workspace: workspaceId,
    projects: [projectId],
    due_on: resolvedDueDate,
  };

  if (sectionId) {
    data.memberships = [{ project: projectId, section: sectionId }];
  }

  return { data };
}

async function findConnectionByAnyId(tenantId) {
...
async function createTask(tenantId, {
  ...
  const connection = await findConnectionByAnyId(tenantId);
  if (!connection) throw new AsanaConnectionMissingError();
  ...
  if (!resolvedProjectId) throw new ValidationError('No project selected. Go to Asana page and save a mapping.');
  ...
  const payload = buildTaskPayload({
    ...
    sectionId: resolvedSectionId,   // may be null — buildTaskPayload handles it
    dueDate,
    workspaceId: resolvedWorkspaceId,
  });

  const connectionDoc = connection;
